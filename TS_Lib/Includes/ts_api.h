...
int 	xdata   TSKey_NoiseThreshold[KEY_CH_COUNT];
#if SUPPORT_ANTI_WATER_FUNCTION
	bit KeyPressedFlag;
	unsigned char xdata PressedKeyIndex;
#endif
#if TK_BASE_TRIM_WHILE_KEY_PRESSED
	unsigned int xdata RefChDataBak[OPENED_TS_COUNT+1];
	unsigned int xdata TS_BaseLineDataBak[OPENED_TS_COUNT+1];
#endif
#else
...
unsigned char xdata Feed_Wdt_Flag;
...
extern unsigned char xdata Feed_Wdt_Flag;
